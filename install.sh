...
echo "========================================="
echo "  Podcop Sub v666 — public install v268"
echo "========================================="
...
echo "========================================="
echo "Podcop Sub v666 public install v268 complete"
echo "Open: Services -> Podkop"
echo "Re-login LuCI after install"
echo "========================================="
# SUBSYNC_INSTALL_VERSION_FILES_V268_BEGIN
echo "========================================="
echo " Podcop Sub v666 OTA v268 safe theme install"
echo "========================================="
...
echo "[1/13] install ProtoByZKS95/proton2025 theme via install.sh"
...
echo "[3/13] use already-installed module JS"
if [ ! -s "$SRC_JS" ] || ! grep -q 'SUBSYNC_DIRECT_REMOVE_MANUAL_HIDE_LOAD_V266B' "$SRC_JS" 2>/dev/null; then
  echo "WARN: local JS missing marker, trying one fallback download before fail"
  if ! wget -qO "$SRC_JS" "$SUBSYNC_RAW_BASE/htdocs/luci-static/resources/view/sub_sync/sub_sync.js?v=$(date +%s)"; then
    echo "ERROR: failed to get sub_sync.js and local JS is not valid"
    exit 1
  fi
fi
...
echo "[5/13] install JS aliases from local tested JS"
for v in 208 211 212 221 238 252 253 254 255 256 258 259 260 261 262 263 264 265 266 267 268; do
...
echo "268" > /etc/sub-sync/module-build
echo "v268" > /etc/sub-sync/module-version
...
nohup sh -c 'sleep 3; /etc/init.d/rpcd restart >/dev/null 2>&1 || true; /etc/init.d/uhttpd restart >/dev/null 2>&1 || true' >/tmp/subsync-v268-delayed-restart.log 2>&1 &

rm -f /tmp/protobyzks95-install.sh 2>/dev/null || true
logger -t sub-sync "Podcop Sub v666 public build v268 installed with safe theme install/uninstall fallback" 2>/dev/null || true

echo "DONE: install.sh finished rc=0"
echo "DONE: Podcop Sub v666 v268 installed. Safe theme install and uninstall fallback enabled."
# SUBSYNC_INSTALL_VERSION_FILES_V268_END
