...
echo "========================================="
echo "  Podcop Sub v666 — public install v359"
echo "========================================="
echo "Backup: disabled for public/friend install"
echo "Downloader: wget strict direct installer v359"
...
echo "=== fix LuCI static JS permissions v359 ==="
...
grep -v '/usr/bin/podcop-sub-v666-guard' /etc/crontabs/root > /tmp/root.cron.v359 2>/dev/null || true
echo '*/5 * * * * /usr/bin/podcop-sub-v666-guard >/tmp/podcop-sub-v666-guard.log 2>&1' >> /tmp/root.cron.v359
cat /tmp/root.cron.v359 > /etc/crontabs/root
rm -f /tmp/root.cron.v359
/etc/init.d/cron restart 2>/dev/null || true

echo "=== version ==="
echo "v359" > /etc/sub-sync/module-version
echo "359" > /etc/sub-sync/module-build
...
echo "DONE_MODULE_OK: Podcop Sub v666 v359 module installed."
echo "DONE_THEME_STATUS: mediaurlbase=$(uci get luci.main.mediaurlbase 2>/dev/null || true)"
echo "DONE: install.sh v359 finished rc=0"
