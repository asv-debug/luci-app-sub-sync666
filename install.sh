...
#!/bin/sh
# PODCOP_SUB_V666_SAFE_PODKOP_RESTART_V390
...
case "$CMD" in
  start|restart|reload)
    ...
    if ! has_outbound; then
      logger -t podcop-sub-v666 "v390: skip podkop $CMD, no outbound configured yet"
      echo "podcop-sub-v666 v390: skip podkop $CMD, no outbound configured yet"
...
echo "========================================="
echo "  Podcop Sub v666 — public install v390"
echo "========================================="
echo "Backup: disabled for public/friend install"
echo "Downloader: wget strict direct installer v390"
...
echo "=== fix LuCI static JS permissions v390 ==="
...
grep -v '/usr/bin/podcop-sub-v666-guard' /etc/crontabs/root > /tmp/root.cron.v390 2>/dev/null || true
echo '*/5 * * * * /usr/bin/podcop-sub-v666-guard >/tmp/podcop-sub-v666-guard.log 2>&1' >> /tmp/root.cron.v390
cat /tmp/root.cron.v390 > /etc/crontabs/root
rm -f /tmp/root.cron.v390
...
echo "DONE_MODULE_OK: Podcop Sub v666 v390 module installed."
echo "DONE_THEME_STATUS: mediaurlbase=$(uci get luci.main.mediaurlbase 2>/dev/null || true)"
echo "DONE: install.sh v390 finished rc=0"
# SUBSYNC_INSTALL_VERSION_FILES_V390_END
