...
#!/bin/sh
# SUBSYNC_SAFE_PODKOP_RESTART_V389
set -u
...
TMP="/tmp/subsync-install-v389.sh"

echo "========================================="
echo " Podcop Sub v666 retry installer v389"
echo "========================================="
...
while [ "$i" -le 10 ]; do
  ...
  if [ -s "$TMP" ] && grep -q 'SUBSYNC_PUBLIC_BUILD_V389' "$TMP" && sh -n "$TMP"; then
    echo "OK: install.sh v389 downloaded and verified"
    sh "$TMP"
    exit $?
  fi

  echo "WARN: downloaded install.sh is not verified v389"
  i=$((i + 1))
  sleep 3
done

echo "ERROR: cannot download verified install.sh v389"
exit 1
