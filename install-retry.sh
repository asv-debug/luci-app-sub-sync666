...
TMP="/tmp/subsync-install-v400.sh"

echo "========================================="
echo " Podcop Sub v666 retry installer v400"
echo "========================================="
...
while [ "$i" -le 10 ]; do
  ...
  if [ -s "$TMP" ] && grep -q 'SUBSYNC_PUBLIC_BUILD_V397' "$TMP" && sh -n "$TMP"; then
    echo "OK: install.sh v400 downloaded and verified"
    sh "$TMP"
    exit $?
  fi

  echo "WARN: downloaded install.sh is not verified v400"
  i=$((i + 1))
  sleep 3
done

echo "ERROR: cannot download verified install.sh v400"
exit 1
